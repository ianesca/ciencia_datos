...
CREATE TABLE tutor (
  ...
  CONSTRAINT fk_tutor_parientes FOREIGN KEY (id_parientes) REFERENCES parentezco(id_parientes)
);
...
CREATE TABLE proceso_seleccion (
  ...
  CONSTRAINT fk_proc_año FOREIGN KEY (id_año) REFERENCES cat_año(id_año),
  CONSTRAINT fk_proc_conv FOREIGN KEY (id_convocatoria) REFERENCES cat_convocatoria(id_convocatoria),
  CONSTRAINT uq_proc UNIQUE (id_programa, id_año, id_convocatoria)
);
...
-------------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION validar_inscripcion_previa(
    p_curp IN VARCHAR2,
...
IS
    v_count NUMBER;
BEGIN
    -- Verificar por CURP en cualquier convocatoria
    SELECT COUNT(*)
    INTO v_count
    FROM aspirante a
    WHERE a.curp = p_curp;
    ...
    -- Verificar por correo en cualquier convocatoria
    SELECT COUNT(*)
    INTO v_count
    FROM aspirante a
    WHERE a.correo_electronico = p_correo;
    
    IF v_count > 0 THEN
        RETURN TRUE;
    END IF;
    
    -- Verificar por clave en cualquier convocatoria
    SELECT COUNT(*)
    INTO v_count
    FROM aspirante a
    WHERE a.id_clave = p_id_clave;
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN TRUE;
END validar_inscripcion_previa;
/

CREATE OR REPLACE FUNCTION validar_inscripcion_mismo_anio(
    p_curp IN VARCHAR2,
    p_id_convocatoria IN NUMBER
) RETURN BOOLEAN
IS
    v_count NUMBER;
    v_anio_convocatoria NUMBER;
BEGIN
    -- Obtener el año de la convocatoria actual
    SELECT c.id_año
    INTO v_anio_convocatoria
    FROM cat_convocatoria c
    WHERE c.id_convocatoria = p_id_convocatoria;
    
    -- Verificar si ya existe inscripción para este CURP en el mismo año
    SELECT COUNT(*)
    INTO v_count
    FROM aspirante a
    JOIN cat_convocatoria c ON a.id_convocatoria = c.id_convocatoria
    WHERE a.curp = p_curp
    AND c.id_año = v_anio_convocatoria;
    
    RETURN (v_count > 0);
EXCEPTION
    WHEN OTHERS THEN
        RETURN TRUE;
END validar_inscripcion_mismo_anio;
/

-------------------------------------------------------------------------------
-- PROCEDIMIENTO DE VALIDACIÓN
-------------------------------------------------------------------------------
CREATE OR REPLACE PROCEDURE validar_aspirante(
    p_curp IN VARCHAR2,
    p_correo IN VARCHAR2,
    p_id_clave IN NUMBER,
    p_id_convocatoria IN NUMBER,
    p_tiene_registro_previo OUT BOOLEAN,
    p_ya_inscrito_mismo_anio OUT BOOLEAN,
    p_puede_continuar OUT BOOLEAN
)
IS
BEGIN
    p_tiene_registro_previo := validar_inscripcion_previa(p_curp, p_correo, p_id_clave);
    p_ya_inscrito_mismo_anio := validar_inscripcion_mismo_anio(p_curp, p_id_convocatoria);
    p_puede_continuar := NOT (p_tiene_registro_previo OR p_ya_inscrito_mismo_anio);
EXCEPTION
    WHEN OTHERS THEN
        p_tiene_registro_previo := TRUE;
        p_ya_inscrito_mismo_anio := TRUE;
        p_puede_continuar := FALSE;
END validar_aspirante;
/

-------------------------------------------------------------------------------
-- TRIGGER PARA PREVENIR DUPLICADOS
-------------------------------------------------------------------------------
CREATE OR REPLACE TRIGGER trg_prevenir_duplicados_aspirante
...
DECLARE
    ...
    v_anio_convocatoria NUMBER;
BEGIN
    -- Obtener el año de la convocatoria
    SELECT c.id_año INTO v_anio_convocatoria
    FROM cat_convocatoria c
    WHERE c.id_convocatoria = :NEW.id_convocatoria;
    
    -- Verificar si ya existe registro para el mismo año
    SELECT COUNT(*)
    INTO v_count
    FROM aspirante a
    JOIN cat_convocatoria c ON a.id_convocatoria = c.id_convocatoria
    WHERE (a.curp = :NEW.curp OR a.correo_electronico = :NEW.correo_electronico OR a.id_clave = :NEW.id_clave)
    AND c.id_año = v_anio_convocatoria;
    
    IF v_count > 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 
            'El aspirante ya tiene una inscripción para este año: ' || v_anio_convocatoria);
    END IF;
END;
/